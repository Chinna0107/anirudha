...
export const BlogPost: React.FC = () => {
  ...
  useSEO({
    ...
    description: post ? post.excerpt : 'Expert insights and architectural guides from Aniruddaya Enterprises.',
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 md:py-20 flex flex-col gap-8">
      ...
      <motion.header 
        ...
        className="flex flex-col gap-4 border-b border-slate-100 pb-6"
      >
        ...
        <TextReveal 
          text={post.title} 
          className="font-display font-extrabold text-3xl md:text-4xl lg:text-5xl text-brand-primary leading-tight" 
        />

        <div className="flex items-center gap-4 text-xs text-slate-500 font-sans mt-2">
      ...
      <motion.article 
        ...
        className="font-sans text-slate-600 leading-relaxed text-sm sm:text-base flex flex-col gap-6"
      >
        {post.content.split('\n\n').map((block, idx) => {
          if (block.startsWith('### ')) {
            return (
              <motion.h3 variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} key={idx} className="font-display font-bold text-xl md:text-2xl text-brand-primary mt-6 mb-1">
                {block.replace('### ', '')}
              </motion.h3>
            );
          }
          if (block.startsWith('* **')) {
            ...
            return (
              <motion.ul variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} key={idx} className="list-disc pl-5 flex flex-col gap-2.5 my-2">
          ...
          return (
            <motion.p variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} key={idx} dangerouslySetInnerHTML={{ __html: block.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />
          );
        })}
      </motion.article>
      ...
      <motion.div 
        ...
        className="bg-slate-50 border border-slate-100 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm mt-12"
      >
